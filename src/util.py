...
def euclidean_dist(x, y):
    ...
    # xx: sum of squares along dim=1 -> shape (m, 1), expand to (m, n)
    xx = torch.pow(x, 2).sum(1, keepdim=True).expand(m, n)
    # yy: same for y, then transpose
    yy = torch.pow(y, 2).sum(1, keepdim=True).expand(n, m).t()
    dist = xx + yy
    # dist - 2 * x @ y.T via addmm_
    dist.addmm_(1, -2, x, y.t())
    # clamp for numerical stability, then sqrt -> pairwise distance matrix
